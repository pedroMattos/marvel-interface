...
async function getAllCharacters(offset = 0, searchTerm = '') {
  const startsWithSearch = `nameStartsWith=${searchTerm}&`
  const hasSearch = !!searchTerm.trim()
  const timestamp = new Date().getTime()
  const md5Hash = md5(timestamp + privateKey + publicKey)
  const { data } = await axios
    .get(`http://gateway.marvel.com/v1/public/characters?limit=10&${hasSearch ? startsWithSearch : ''}offset=${offset}&ts=${timestamp}&apikey=${publicKey}&hash=${md5Hash}`)
